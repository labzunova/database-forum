...
-- --CREATE INDEX IF NOT EXISTS users_nick_id ON users (nickname, id); -- get id ny nick
-- -- --CREATE INDEX IF NOT EXISTS user_nickname on users using hash(nickname); -- NEW
-- --CREATE INDEX IF NOT EXISTS user_nickname on users using hash(nickname); -- primary key так шо не надо, но чекнуть еще
--
-- -- FORUMS --
-- CREATE UNLOGGED TABLE forums (
...
-- );
-- CREATE INDEX IF NOT EXISTS thread_forum_and_created ON threads (forum, created); -- для get forum threads
-- CREATE INDEX IF NOT EXISTS thread_forum ON threads using hash(forum); -- для get forum threads
-- CREATE INDEX IF NOT EXISTS thread_slug ON threads using hash(slug) where slug != '';
-- -- --CREATE INDEX IF NOT EXISTS thread_slug ON threads (lower(slug));
-- -- --CREATE INDEX IF NOT EXISTS thread_forum ON threads USING hash (forum);
-- CREATE INDEX IF NOT EXISTS thread_created ON threads (created);
...
--
-- -- три ниже это я пыталась изменить parent tree запрос
-- -- CREATE INDEX IF NOT EXISTS post_path ON posts (path);
-- -- CREATE INDEX IF NOT EXISTS post_parent_and_thread ON posts (parent, path); -- для parentTree сортирвки
-- -- CREATE INDEX IF NOT EXISTS post_pathFirst_parent_and_thread ON posts (thread, (path[1]), id); -- для parentTree сортирвки
--
-- -- -- --CREATE INDEX IF NOT EXISTS post_id_path ON posts (id, path); -- !запрос для flat сортировки
-- -- CREATE INDEX IF NOT EXISTS post_thread_and_parent ON posts (thread, parent); -- не робiт
-- -- CREATE INDEX IF NOT EXISTS post_path ON posts (path); -- не робiт
--
-- -- create index if not exists post_thread_parent on posts (thread, parent);
-- --create index if not exists post_thread_parent on posts (thread, parent, path);
-- create index if not exists post_thread_parent on posts (thread, parent, path, id);
--
--  CREATE INDEX IF NOT EXISTS post_id_path ON posts (id, (path[1]));
--
-- create index if not exists post_pathOne_id_parent on posts ((path[1]), path, id);
-- create index post_path on posts using gin (path);
--
-- CREATE OR REPLACE FUNCTION new_post_added() RETURNS TRIGGER AS
-- $new_post_added$
-- begin
