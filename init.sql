...
--CREATE INDEX IF NOT EXISTS user_nickname on users using hash(nickname); -- primary key так шо не надо
-- CREATE INDEX IF NOT EXISTS users ON users (nickname) include (fullname, about, email);
...
-- CREATE INDEX IF NOT EXISTS forums_slug on forums using hash (slug); primary key так шо не надо
...
--CREATE INDEX IF NOT EXISTS forum_users ON forum_users (forumSlug) include (usernickname, about, email, usernickname);
